...
func saveCertToDB(cert *JSONCertificate, userEmail string) error {
	...
	certificateParams := certsSQL.CreateCertificateParams{
		...
		SignatureAlgorithm:        sql.NullString{String: cert.SignatureAlgorithm, Valid: true},
		PublicKeyAlgorithm:        sql.NullString{String: cert.PublicKeyAlgorithm, Valid: true},
		PublicKeySize:             sql.NullInt64{Int64: int64(cert.PublicKeySize), Valid: true},
